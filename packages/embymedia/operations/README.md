...
- **Share access codes are process-local** — resource search exposes only a session-bound candidate ID; candidate or plan expiry and service restart discard the value, so an unexecuted protected-share update must start from a fresh search. Persisted confirmations and final verification never recover or expose the code.
