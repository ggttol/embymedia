...
- Automatic episode completion refreshes each followed Series' Emby metadata before listing missing episodes, so newly published TMDB episode dates become visible instead of waiting for Emby's provider cache to expire; refresh failures degrade to cached data with a visible task log entry.
