...
async function probeMCP(runSafeCalls: boolean) {
  ...
  try {
    const initialized = await postMCP({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-03-26', capabilities: {}, clientInfo: { name: 'embymedia-web-diagnostic', version: '2.0.0' } } }); if (initialized.result?.error) throw new Error(initialized.result.error.message); const sessionId = initialized.response.headers.get('Mcp-Session-Id') ?? ''; await postMCP({ jsonrpc: '2.0', method: 'notifications/initialized', params: {} }, sessionId); phases.value[0] = { name: '协议初始化', state: 'online', detail: 'initialize 与 initialized 已完成' }
