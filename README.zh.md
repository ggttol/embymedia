...
- 已核验的媒体导入：夸克分享可选经直连 NAS Worker 下载并发布到 115；每个文件在传输阶段完成前都按父目录、名称、大小与 SHA-1 核对。
- 自动编目入库：只有唯一证明一个现有 Emby Series 时，已核验导入才继续移动、规范化命名、同步 STRM、跟踪 Emby 扫描并复核已播集号。
- 已播缺集补全：按计划检索 115 与夸克，为两个追更媒体库补齐缺集；候选冲突作为明确发现项报告，不再让其他剧集的有效任务失败。
- 任务中心：一条七阶段工作流视图（来源、NAS 传输、115 核对、编目、整理、STRM、Emby），附带持久尝试、取消、重试与审计状态。
- 每小时 Emby 快速刷新加每日一次完整 STRM 核对，让例行扫描保持快速并保留验证。
