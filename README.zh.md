...
- 列出 Emby 媒体库，按确切条目 ID 检查，提交指定媒体库刷新，通过 Emby 的 scheduled-task 状态跟踪全库扫描直至记录完成，应用明确的 TMDB identity 与图片，并返回缺失海报的完整总数，以及最多 100 个路径和 provider ID。
- 从已配置媒体树同步 STRM 文件且不跟随输出 symlink；只有媒体挂载 canary 存在时，才删除目标已消失的生成 STRM 文件；并验证每个剩余 target 均位于媒体根目录内且真实存在。
