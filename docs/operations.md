...
For two-episode updates, open the Quark share dialog, read the recursive preview, select only the two files, and check the selected count and bytes before submitting. REST uses `POST /api/v1/drive/share-snapshot` with `recursive:true`, then `POST /api/v1/quark/share-imports` with `selected_source_manifest:[{id,revision,name,size},...]`; MCP uses `quark_snapshot_share` then `quark_import_share_to_115`. Empty selection is an error, not full import. Whole-pack downloads require `import_all:true`; old clients and persisted whole-pack tasks without that explicit consent are rejected. Do not work around this by moving files to another Quark directory or changing the production database.

To use a supplied link for automatic missing-episode completion, submit `series_auto_fill` with `{"libraries":["电视剧追更"],"series_ids":["EMBY_SERIES_ID"],"source_shares":{"EMBY_SERIES_ID":{"provider":"quark","url":"https://pan.quark.cn/s/SHARE_CODE"}},"transfer":true}`. Use `transfer:false` to preview. `candidate_overrides` takes resource-index IDs, not share codes; `/api/v1/search` uses `q`, and rejects the unsupported `keyword` parameter. Matching still requires trusted series identity and aired gaps; a supplied URL does not authorize replacement of existing episodes. Failed or ambiguous selection stops without falling back to the pack.

Keep transfers on the durable worker path. Directly streaming a CDN response into a final FUSE media filename can expose a partial video; do not generate STRM from its existence or approximate size. Verify exact size and SHA-1 through 115 before media ingest. HTTP 412 credential recovery is available in both Debian and the NAS worker; deploy the matching worker binary as well as the server to enable the NAS fix. A failed preflight without an import row can be retried; an import row without reconciled saved roots requires review, not automatic resave.
...
After every manually submitted Quark import reaches verified 115 identities, the controller creates one deterministic `media_ingest` continuation. No separate schedule is required. The continuation moves only numbered video files and related subtitles when one existing Emby Series is uniquely proven by title plus year or TMDB evidence; unsupported files and ambiguous matches remain in `_待整理`. It then synchronizes the matching library's STRM output, waits for an Emby scan, and verifies the imported aired episode numbers. Task Center groups both durable tasks into one seven-stage workflow. Retrying the transfer does not duplicate the continuation; retry the failed current stage from the grouped workflow.
