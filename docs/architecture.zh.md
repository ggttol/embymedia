...
手动夸克导入必须提供非空 `selected_source_manifest`，记录准确文件叶子的 ID、修订标识、名称与大小；整包导入必须明确设置 `import_all:true`。先无写入递归预览，再于选择性转存前重新校验，下载前拒绝多余、缺失或有歧义的已转存文件。夸克目标目录只是保存位置，不是选集过滤器。未声明任一模式的旧持久任务安全失败；只有不存在前次导入检查点行时，重试才允许重新转存，否则必须协调已保存根与清单。NAS Worker 在 HTTP 412 时刷新过期 CDN 凭据并校验分片响应；STRM 可见或 Emby 媒体源数量均不能证明文件完整可播。

完整同步执行一次有序的 CloudDrive FUSE 源遍历、协调 STRM 输出、验证每个目标、请求 Emby 扫描并等待 provider 完成；源扫描进度按上次本地 STRM 清单估算，每处理 500 个媒体文件更新一次。每小时计划使用 `sync_strm=false`，只运行可跟踪的 Emby 扫描；由于生产 CloudDrive webhook 可能被禁用，每日仍保留一次完整同步作为最终一致性兜底。应用内部的媒体修改继续在 Emby 验证前执行既有的范围化 STRM 同步。STRM 协调保留原始媒体路径，拒绝有歧义的规范目标，要求挂载 canary 可读后才能移除过期输出，并通过文件系统 ACL 保持 Emby 共享访问权限。

自动补集分别搜索 115 与夸克，并通过实时分享树判断候选是否可用，不按资源索引的健康标记筛选。在缺集扫描之前，资格媒体库中每个绑定 TMDB 的 Series 都会收到一次尽力而为的 Emby 元数据刷新请求，使 TMDB 在 Emby 缓存快照之后发布的集数重新可见；Emby 自身的 provider 节流仍可能延迟一个周期，刷新失败只记录日志不中断执行。预检模式不执行 provider 写操作，并报告包含 provider 的证据；同一集有多个可用资源时保持明确冲突，直到操作者选择资源。普通单集要求实时路径使用规范标题或原始 Series 标题；候选与分享元数据只能佐证规范年份或 TMDB ID，因此嘈杂的元数据标题不能覆盖实时路径。选中的 115 叶子继续直接进入准确目录；选中的夸克叶子由持久子任务处理：服务器在转存分享前验证并持久保存已配置的 115 媒体库、规范 Emby Series、准确目标目录、源 ID 与预期集号，随后恢复下载和上传检查点，并且仅在 115、STRM 与 Emby 均验证后完成。公开任务请求不能提供内部目标绑定。不可用分享、未匹配集数与候选冲突作为明确发现项完成，不再让整次计划失败或取消其他 Series 的有效任务。取消、限流、账号授权、传输故障、provider 5xx、配置错误及写入后验证失败仍是真实任务错误；每小时计划提供有界的稍后重试，而不是立即持续请求 provider。[provider 限定补集决策](../.agents/notes/implemented/feature/2026-09-13-provider-qualified-episode-completion.zh.md)定义这些边界。

`series_auto_fill.source_shares` 将用户提供的 HTTPS 分享绑定到准确 Emby Series ID，并对这些来源跳过资源索引检索；未提供 `series_ids` 时，范围仅限这些键。仍执行既有规范身份与已播缺集检查；未知 Series ID、身份不符、选集未匹配时，绝不扩大为整包下载。订阅任务将目标 CID 解析为唯一现有追更 Series，排入同一范围化流程，并报告 `next_task_id` 而不是宣称完成。普通 115 分享过期属于单剧集发现项，不再取消其他剧集已排队的夸克传输；取消、鉴权、限流、系统性传输错误及不安全的写后验证仍明确失败。
