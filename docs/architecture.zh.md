...
队列持久保存等待执行的工作与尝试结果；自动计划创建关联执行。中断的有副作用任务会失败并等待明确检查，不会自动重放 provider 修改。provider 可能在进程记录完成前已经接受操作，因此本地失败不能证明没有副作用。
...
自动补集通过实时 115 分享内容判断候选是否可用，不按资源索引的健康标记筛选。仍有缺集时，检查失败会逐项标明资源；分享可读但没有合适剧集属于普通资源缺口。预检模式不转存文件，并分别报告 Emby 实际缺集与匹配资源。普通单集要求已检查路径使用规范标题或原始 Series 标题；候选与分享元数据只能佐证规范年份或 TMDB ID，因此嘈杂的元数据标题不能覆盖实时路径。[实时路径身份决策](../.agents/notes/implemented/bug-fix/2026-09-10-live-share-path-identity.zh.md)定义这些证据的作用。
