...
Manual Quark imports require a non-empty `selected_source_manifest` of exact file leaves (ID, revision, name, size), or explicit `import_all:true`. Preview recursively without provider writes; revalidate before selective save, then reject extra, missing, or ambiguous saved leaves before any download. The target Quark directory is a destination, never a selection filter. Persisted tasks without either mode fail closed; retries may save anew only when no prior import checkpoint row exists, otherwise saved roots and manifest must reconcile. The NAS worker refreshes expired CDN credentials on HTTP 412 and validates ranged responses; neither a visible STRM nor an Emby media-source count proves a complete playable file.
...
`series_auto_fill.source_shares` binds supplied HTTPS shares to exact Emby Series IDs and bypasses resource-index lookup for those sources. With no `series_ids`, scope is limited to the supplied keys. Existing canonical identity and aired-missing-episode checks still apply; unknown Series IDs, mismatched identity, and unmatched selections never widen into whole-pack downloads. Subscription tasks resolve their target CID to one existing following Series and enqueue this same scoped workflow, reporting `next_task_id` rather than claiming completion. An expired ordinary 115 share is a per-Series finding and does not cancel another Series' queued Quark transfer; cancellation, authentication, rate limits, systemic transport errors, and unsafe post-write verification remain failures.
