...
Worker 按从旧到新的顺序调度持久化 pending 执行，因此服务重启后仍保持计划顺序。STRM 同步和验证在路径发现后使用两个有界 worker；更高并发会使生产 FUSE 路径过载并延长完成时间。普通镜像输出冲突时，会在同一个 worker 中保留源目录遍历顺序；规范季目录的输出则会拒绝不同源文件映射到同一个目标。验证会报告单调递增的已处理文件进度，在第一个错误出现时取消另一个 worker，等待两个 worker 停止，并返回确定且有数量限制的示例。

生产自动计划将每小时 00 分留给自动补集，将 05 分留给一次合并的 STRM 同步和 Emby 刷新。独立的每小时同步和事件触发全库刷新均已停用，因为一次全量同步约需 20 分钟，而额外刷新会重复此工作。元数据修复、海报修复和只读 STRM 验证每天 03:40、03:45 和 04:40 执行，避开每小时扫描窗口。

过期 STRM 协调会记录它所删除生成文件的每个上级目录，再从最深层开始，只删除仍为空的目录。它绝不删除无关的非空目录。随后的 Emby 扫描因此会停止索引已删除 Series，而不是保留其空目录和缓存海报。

旧式 `SEnn` 目录仅在直接包含的视频具有一个明确且匹配的季集标记时，才规范为 `Season NN`。输出文件名保留剧集上级目录和发布标记，文件内容仍指向原始源路径。编号有歧义或规范输出冲突时会报错，而不是选择某个源或身份。协调过程仅在规范目标中已存在完全相同的生成内容且挂载 canary 存在时，才移除旧镜像名称；编辑过的镜像保持不变。按源路径指定季范围时，同步和验证均定位到规范输出，重复同步不会重新生成旧名称。
...
Release 安装在停止应用前最多等待一小时，让当前后台执行完成。等待中的执行会跨重启持久保留，systemd 只启动一次新应用进程。无法在限时内完成的任务会让当前 release 保持不变。需要立即激活时，显式的 `EMBYMEDIA_DEPLOY_FORCE=1` override 会改为取消正在运行的执行。安装在刷新 webhook 凭据时保留现有启用开关。内容完全相同的 CloudDrive webhook 配置会保留 inode，因此部署不会加入错误的入库任务。
...
**通过重命名 Emby 条目或源视频修复季分组。** 仅修改元数据无法纠正季集解析，而重命名源媒体会影响独立消费者。规范生成名称能够保留源路径；保留剧集上级目录则避免猜测相似名称的发布目录属于同一部剧。
